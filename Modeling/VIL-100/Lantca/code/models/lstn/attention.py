...
class MultiheadAttention(nn.Module):
    def __init__(self,
                 ...
                 d_att=None,
                 use_dis=False,
                 qk_chunks=1, #在Q和k做乘法的时候，是不是把k分成这些块
                 max_mem_len_ratio=-1,
                 top_k=-1):
        super().__init__()
    ...
    def forward(self, Q, K, V):
        ...
        outputs = multiply_by_xchunks(attn, V, self.qk_chunks).permute(2, 0, 1, 3)#就是在计算attn*V
...
class MultiheadLocalAttentionV2(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        else:  #其中pad_and_unfold得到一个(num_windows, window_size * window_size) 然后改变形状为（1，1，225，3840）
            memory_mask = torch.ones((1, 1, h, w), device=v.device).float()
        ...
        #q（2，128，48，80）-》（2，1800，48，80）
        relative_emb = self.relative_emb_k(q)  # conv(256, 1800, k1s1g1) [2, 1800, 30, 30]      #b=1

        # Scale
        q = q / self.T

        q = q.view(-1, self.d_att, h, w)  # [16, 32, 30, 30]            #torch.Size([16, 16, 48, 80])           #报错时为：torch.Size([8, 16, 48, 80])
        k = k.view(-1, self.d_att, h, w)  # [16, 32, 30, 30]            #torch.Size([16, 16, 48, 80])
        v = v.view(-1, self.num_head, hidden_dim, h*w)  # [2, 8, 32, 900]           #torch.Size([2, 8, 16, 3840])

        relative_emb = relative_emb.view(n, self.num_head, self.window_size*self.window_size, h*w)  # [2, 8, 225, 900]      #torch.Size([2, 8, 255, 3840]),torch.Size([2, 1800, 48, 80])
