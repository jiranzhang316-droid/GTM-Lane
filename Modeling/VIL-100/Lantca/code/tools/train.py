    ...
    def training(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            for t in range(self.cfg.clip_length, -1, -1):
                ...
                self.model.clip_idx = self.cfg.clip_length - t #0,1,2
                ...
                if self.cfg.clip_length - t >= self.cfg.num_t:     
                    self.model = self.vm.forward_for_dict_transfer(self.model)  #创建了一个视频大于第一帧的时候自己记忆
                    ...
                    self.vm.forward_for_dict_update_per_frame(self.model, mode='update') #将之前存的结果修改掉
                ...
                out_post = self.post_process.run_for_training(out[key_t])#这个其实是后处理了，不用管（不过有nms可以尝试调整为图网络）
