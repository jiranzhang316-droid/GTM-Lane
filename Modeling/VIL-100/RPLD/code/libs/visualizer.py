...
class Visualize_cv(object):
    ...
    def draw_selected_lane_coords(self, x_coords, height_idx=None, color=(0, 255, 0), s=2):
        ...
        if len(x_coords) != 0:
            for i in range(len(x_coords)):
                ...
                self.draw_polyline_cv(lane_coords, name='temp', ref_name='temp', color=(255, 255, 255), s=5)
    ...
    def display_for_test(self, batch, out, prev_frame_num, batch_idx, mode):
        ...
        self.show['coeff_cls_map'] = self.draw_selected_lane_coords(out['x_coords'], height_idx=out['height_idx'],
                                                                    color=(0, 255, 0), s=2)
        ...
        # 2. 定义新的保存路径 (建议换个文件夹名叫 masks 或 paper_imgs，避免和之前的混合)
        save_path = f'{self.cfg.dir["out"]}/{mode}/masks/{dirname}/{filename}.jpg'

        # 3. 直接保存纯粹的模型预测图 (黑底白线)
        # 方案A：保存平滑的结构化曲线 (推荐，线条更清晰锐利，适合论文)
        self.save_data(path=save_path, data=self.show['coeff_cls_map'])

        # 方案B：如果你更想要原始的分割掩码像素图，可以解注下面这行，注释掉上面那行
        # self.save_data(path=save_path, data=self.show['seg_map'])
        # ================= 修改结束 =================
