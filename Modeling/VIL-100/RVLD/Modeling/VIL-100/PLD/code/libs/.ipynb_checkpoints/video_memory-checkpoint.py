...
class Video_Memory(object):
    ...
    def forward_for_dict_initialization(self):
        self.keylist = ['img_feat', 'prob_map', 'coeff_map', 'guide_cls', 'queries']
        self.data = dict()
    ...
    def forward_for_dict_update_per_frame(self, model, batch_idx=None, mode=None):
        if mode == 'intra' and batch_idx is not None:
            ...
            if hasattr(model, 'query_output') and 't-0' in model.query_output:
                self.data['queries'][self.t] = model.query_output['t-0']
