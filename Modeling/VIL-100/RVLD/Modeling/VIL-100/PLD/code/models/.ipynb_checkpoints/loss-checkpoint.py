...
class Loss_Function(nn.Module):
    ...
    def forward(self, out, gt, t, epoch):
        ...
        # flow / warping loss (only meaningful in dense optical-flow paradigm)
        if out[t].get('grid') is not None:
            ...
            # Sparse query paradigm: no dense flow warping. Set to zero.
            loss_dict['warping_c'] = torch.FloatTensor([0.0]).cuda()
