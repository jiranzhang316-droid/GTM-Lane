...
class Model(nn.Module):
    ...
    def forward_for_regression(self):
        ...
        feat_c = self.feat_embedding(self.prob_map.detach()) #self.prob_map.detach()的作用是复制一份，然后将原来的1车道，通道扩展为64
        feat_c = feat_c + self.pe.expand(b, -1, -1, -1) #加入一个初始化位置编码 但是不改变形状（4，64，96，160）

        offset = self.offset_regression(feat_c) #激活函数 同时缩减通道 64->18

        x = self.regressor(feat_c)# 回归 输出（4，64，96，160） 作用是 从特征图预测每个位置的卷积偏移参数 表示 局部车道形状信息
        coeff_map = self.deform_conv2d(x, offset) #每个像素点对应 top_m = 6个车道线参数 输出（4，6，96，160）
