...
class Train_Process(object):
    ...
    def training(self):
        loss_t = dict()
        # train start
        self.model_s.eval() #预训练好的seg部分不更新
        self.model_c.eval() #预训练好的coeff部分不更新
        self.model.train() # 联合的 seg的分类层+整个coeff部分 进行更新
        print('train start')
        logger('train start\n', self.logfile)

        self.finetune_model()
        # 其中有 img_rgb 用于可视化原始图片 img：用于输入到模型中 org_label：原始的车道线标签  seg_label:第一阶段的标签  coff_label:第二阶段的标签  visit:有效的像素mask  guide_mask_neg:负向mask
        for i, batch in enumerate(self.dataloader):
            ...
            for t in range(self.cfg.clip_length, -1, -1):
                ...
                self.vm.forward_for_dict_initialization_per_frame(t='t-0') #对t-0时刻的字典进行初始化形成self.data[key][t] = dict()
                self.model_s.forward_for_encoding(batch[key_t]['img']) #这里第一次传入 t- *时刻的图片 用resnet提取特征
                self.model_s.forward_for_squeeze()#主要做特征融合
                out[key_t].update(self.model_s.forward_for_classification())#获取预测的车道线输出
                self.model_c.prob_map = self.model_s.prob_map[:, 1:] #model_s.prob_map里面有两个，取得是1第二个，也就是预测结果，第一个是概率
                out[key_t].update(self.model_c.forward_for_regression()) #回归车道线系数

                self.vm.forward_for_dict_update_per_frame(self.model_s, mode='intra')
                if self.cfg.clip_length - t >= self.cfg.num_t: # 如果不是第一帧走下面
                    self.model = self.vm.forward_for_dict_transfer(self.model) #初始化一个记忆存储
                ...
                # lane mask guide
                self.post_process.mode = ('f' if self.cfg.clip_length - t >= self.cfg.num_t else 'init') #如果是第一帧 mode = init 不是 mode = f
                out_post = self.post_process.run_for_training(out[key_t]) #得到处理后的车道线坐标等等
