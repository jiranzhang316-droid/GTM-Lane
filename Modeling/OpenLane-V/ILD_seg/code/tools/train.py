...
class Train_Process(object):
    def __init__(self, cfg, dict_DB):
        ...
        self.cfg.iteration['validation'] = len(self.dataloader) // 3
        self.cfg.iteration['record'] = self.cfg.iteration['validation'] // 6
    ...
    def training(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            # 对提取出来的特征做压缩，例如 通道压缩 多尺度融合
            self.model.forward_for_squeeze()
            # 真正输出预测结果，并且将结果放到out里面
            ...
            if i % self.cfg.disp_step == 0:
                ...
                # 输出图片的可视化对比结果，训练可视化每次只显示 batch 的第一张图片
