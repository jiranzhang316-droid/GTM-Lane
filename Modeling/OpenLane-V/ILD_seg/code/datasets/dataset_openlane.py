...
class Dataset_Test(Dataset):
    def __init__(self, cfg, mode=None):
        self.cfg = cfg
        self.mode = 'training' if 'val' in mode else 'validation'
        self.datalist = load_pickle(f'{self.cfg.dir["dataset"]}/OpenLane-V/list/datalist_{self.mode}')
    ...
    def get_data_org(self, idx):
        # 读取训练图片
        img = Image.open(f'{self.cfg.dir["dataset"]}/images/{self.mode}/{self.datalist[idx]}.jpg').convert('RGB')
        # 读取对应的标签
        anno = load_pickle(f'{self.cfg.dir["dataset"]}/OpenLane-V/label/{self.mode}/{self.datalist[idx]}')
        # 图像裁剪 + 标注同步裁剪
        img, anno = self.cropping(img, anno)
        return img, anno
