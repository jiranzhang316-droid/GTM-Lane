...
class Dataset_Train(Dataset):
    ...
    def get_data_preprocessed(self, data):
        # initialize error case 这里给当前图片（img_name）初始化一个 错误标记容器，用于后续标记 车道线缺少、数据异常、标注错误
    ...
    def __init__(self, cfg, mode=None):
        ...
        if cfg.sampling == True:
            if cfg.sampling_mode == 'video':
                ...
                self.datalist = list(datalist_video_out)
            else:
                sampling = np.arange(0, len(self.datalist), cfg.sampling_step)
                self.datalist = np.array(self.datalist)[sampling].tolist()

        save_pickle(path=f'{self.cfg.dir["out"]}/{mode.replace("eval", "test")}/pickle/datalist', data=self.datalist)
