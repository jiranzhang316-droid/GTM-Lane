...
class Model(nn.Module):
    ...
    def forward_for_feat_aggregation(self, is_training=False):      
        ...
        key_t = f't-0'     #当前帧的时刻信息
        query_img_feat = self.memory['img_feat'][key_t]        

        curr_emb = self.conv_layer(query_img_feat)     #通道对齐
        batch_size = query_img_feat.size()[0]  #获取batch_size
        enc_hw = curr_emb.size()[2]*curr_emb.size()[3] #获取窗口大小
        self.size_2d = curr_emb.size()[2:]

        lane_mask=self.memory['guide_cls']['t-1']       #取出上一帧的时刻记忆
        curr_one_hot_mask = one_hot_mask(lane_mask, 1)  #生成二值掩码
        if not self.memory['long']['t-1']:        #如果没有长时记忆，也就是第二帧的话构造 空间位置编码，形状变成[HW, B, C]
            ...
            self.memory['pos_emb']['t-0'] = self.pos_emb  #根据车道 mask 分配 identity embedding，给不同 lane token 一个“身份提示”
        else:          #如果不是第一帧就不用再重新构造身份信息，
            curr_id_emb=None   
            self.memory['pos_emb']['t-0']=self.memory['pos_emb']['t-1'] #继承之前的位置编码
            self.update_short_term_memory(self.lsab_curr_memorie, batch_size, enc_hw)    #更新短期记忆缓存
        
        n, c, h, w = curr_emb.size()
        _curr_emb = curr_emb.view(n, c, h*w).permute(2, 0, 1) #也就是 Transformer 风格的 token 序列

        self.long_term_memories=self.memory['long']['t-1'] #读取历史累计记忆信息
        # lsab_embs：聚合后的当前帧特征  lsab_memories：更新后的 memory
        lsab_embs, lsab_memories = self.LSAB(
            _curr_emb, #当前帧的查询信息
            self.long_term_memories, #长时记忆
            self.short_term_memories, #短时记忆
            curr_id_emb, #lane identity 车道位置提示 只有第二帧才有，后面的帧是没有的
            self.memory['pos_emb']['t-0'], #当前帧的车道位置编码，会不断更新
            self.size_2d #ab_embs：聚合后的当前帧特征
        )
        ...
        return {'key_probmap': lane_mask,           
                ...
                'grid': None,
                'long':self.long_term_memories,#长时记忆
                'pos_emb':self.pos_emb,
                'short':self.short_term_memories #短时记忆
                }
